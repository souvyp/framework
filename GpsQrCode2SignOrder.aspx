		...
		<div id="main">
			<div class="inp">
				<div class="inp1"> 
					...
					<input id="telephonenumber" type="text" placeholder="请输入您的手机号">
				</div>
				<button class="subbtn" onclick="telephone()">确认</button>
			</div>
		</div>
	</body>
	<script>
		function telephone(){
			...
			var _DeviceCode = NSF.UrlVars.Get('code', location.href);
			var _openid = $('input#OpenId').val();
			
			var vml = '[{"namespace":"protocol","cmd":"data","version":1,"id":"Dtms_0108","paras":[{"name":"OpenID","value":"'+_openid+'"},{"name":"tocontact","value":"'+Tel+'"},{"name":"devicecode","value":"'+_DeviceCode+'"}]}]';
			...
			function(result, data) {
				if(result) {
					if(data[0].rs[1].rows==""){
						alert("手机号码不匹配");
					}else if(data[0].rs[1].rows!==""){
						var orderid = data[0].rs[1].rows[0].orderid;
						window.location.href="/GpsQrCode2SignOrderDetail.aspx?OpenId="+_openid+"&DeviceCode="+_DeviceCode+"&tel="+Tel+"&id="+orderid;
					}
				}
			})
		}
